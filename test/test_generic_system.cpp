...
public:
  ...
  FRIEND_TEST(TestGenericSystem,
              valid_urdf_ros2_control_system_robot_with_gpio);
  FRIEND_TEST(TestGenericSystem,
              valid_urdf_ros2_control_system_robot_with_gpio_mock_command);
  FRIEND_TEST(TestGenericSystem,
              valid_urdf_ros2_control_system_robot_with_gpio_mock_command_True);
  ...
  explicit TestableResourceManager(rclcpp::Node &node, const std::string &urdf,
      ...
      : hardware_interface::ResourceManager(
            ...
            node.get_node_logging_interface(), activate_all, 100) {}
};

void set_components_state(TestableResourceManager &rm,
...
auto configure_components =
    [](TestableResourceManager &rm,
      ...
      set_components_state(rm, components,
...
TEST_F(TestGenericSystem, generic_system_2dof_symetric_interfaces) {
  ...
  hardware_interface::LoanedStateInterface j1p_s =
      rm.claim_state_interface("joint1/position");
  hardware_interface::LoanedStateInterface j2p_s =
      rm.claim_state_interface("joint2/position");
  hardware_interface::LoanedCommandInterface j1p_c =
      rm.claim_command_interface("joint1/position");
  hardware_interface::LoanedCommandInterface j2p_c =
      rm.claim_command_interface("joint2/position");
...
TEST_F(TestGenericSystem, generic_system_2dof_asymetric_interfaces) {
  ...
  hardware_interface::LoanedStateInterface j1v_s =
      rm.claim_state_interface("joint1/velocity");
...
void generic_system_functional_test(
  ...
  hardware_interface::LoanedStateInterface j2p_s =
      rm.claim_state_interface("joint2/position");
  hardware_interface::LoanedStateInterface j2v_s =
      rm.claim_state_interface("joint2/velocity");
  hardware_interface::LoanedCommandInterface j1p_c =
      rm.claim_command_interface("joint1/position");
  hardware_interface::LoanedCommandInterface j1v_c =
      rm.claim_command_interface("joint1/velocity");
  hardware_interface::LoanedCommandInterface j2p_c =
      rm.claim_command_interface("joint2/position");
  hardware_interface::LoanedCommandInterface j2v_c =
      rm.claim_command_interface("joint2/velocity");
...
void generic_system_error_group_test(const std::string &urdf,
  ...
  for (auto component : {component1, component2}) {
    EXPECT_EQ(status_map[component].state.label(),
              hardware_interface::lifecycle_state_names::UNCONFIGURED);
  ...
  if (validate_same_group) {
    // If they belong to the same group, show the error in all hardware
    // components of same group
    EXPECT_THAT(read_result.failed_hardware_names,
                ::testing::ElementsAre(component1, component2));
  } else {
    // If they don't belong to the same group, show the error in only that
    // hardware component
    EXPECT_THAT(read_result.failed_hardware_names,
                ::testing::ElementsAre(component1));
  }
  ...
  status_map = rm.get_components_status();
  EXPECT_EQ(status_map[component1].state.label(),
            hardware_interface::lifecycle_state_names::INACTIVE);
  EXPECT_EQ(status_map[component2].state.label(),
            hardware_interface::lifecycle_state_names::INACTIVE);
}
...
TEST_F(TestGenericSystem,
       generic_system_2dof_error_propagation_different_group) {
  auto urdf =
...
TEST_F(TestGenericSystem, generic_system_2dof_sensor) {
  ...
  hardware_interface::LoanedStateInterface j2v_s =
      rm.claim_state_interface("joint2/velocity");
  hardware_interface::LoanedStateInterface sfx_s =
      rm.claim_state_interface("tcp_force_sensor/fx");
  hardware_interface::LoanedStateInterface sfy_s =
      rm.claim_state_interface("tcp_force_sensor/fy");
  hardware_interface::LoanedStateInterface stx_s =
      rm.claim_state_interface("tcp_force_sensor/tx");
  hardware_interface::LoanedStateInterface sty_s =
      rm.claim_state_interface("tcp_force_sensor/ty");
  hardware_interface::LoanedCommandInterface j1p_c =
      rm.claim_command_interface("joint1/position");
  hardware_interface::LoanedCommandInterface j2p_c =
      rm.claim_command_interface("joint2/position");
...
TEST_F(TestGenericSystem, hardware_system_2dof_with_mimic_joint) {
  auto urdf = ros2_control_test_assets::urdf_head_mimic +
              hardware_system_2dof_with_mimic_joint_ +
              ros2_control_test_assets::urdf_tail;
...
TEST_F(TestGenericSystem,
       generic_system_2dof_functionality_with_offset_custom_interface_missing) {
  auto urdf =
      ros2_control_test_assets::urdf_head +
      hardware_system_2dof_standard_interfaces_with_custom_interface_for_offset_missing_ +
      ros2_control_test_assets::urdf_tail;
...
TEST_F(TestGenericSystem,
       generic_system_2dof_functionality_with_offset_custom_interface) {
  auto urdf =
      ros2_control_test_assets::urdf_head +
      hardware_system_2dof_standard_interfaces_with_custom_interface_for_offset_ +
      ros2_control_test_assets::urdf_tail;
  ...
  // check is hardware is configured
  auto status_map = rm.get_components_status();
  EXPECT_EQ(status_map[hardware_name].state.label(),
            hardware_interface::lifecycle_state_names::UNCONFIGURED);
...
TEST_F(TestGenericSystem, gpio_with_initial_value) {
  ...
  hardware_interface::LoanedStateInterface state =
      rm.claim_state_interface("sample_io/output_1");

  ASSERT_EQ(1, state.get_value());
}

TEST_F(TestGenericSystem,
       simple_dynamics_pos_vel_acc_control_modes_interfaces) {
  auto urdf =
      ros2_control_test_assets::urdf_head +
      hardware_system_2dof_standard_interfaces_with_different_control_modes_ +
      ros2_control_test_assets::urdf_tail;
  ...
  hardware_interface::LoanedStateInterface j1v_s =
      rm.claim_state_interface("joint1/velocity");
  hardware_interface::LoanedStateInterface j1a_s =
      rm.claim_state_interface("joint1/acceleration");
  hardware_interface::LoanedStateInterface j2p_s =
      rm.claim_state_interface("joint2/position");
  hardware_interface::LoanedStateInterface j2v_s =
      rm.claim_state_interface("joint2/velocity");
  hardware_interface::LoanedStateInterface j2a_s =
      rm.claim_state_interface("joint2/acceleration");
  hardware_interface::LoanedCommandInterface j1p_c =
      rm.claim_command_interface("joint1/position");
  hardware_interface::LoanedCommandInterface j1v_c =
      rm.claim_command_interface("joint1/velocity");
  ...
  ASSERT_EQ( // joint2 has non 'position', 'velocity', or 'acceleration'
             // interface
      rm.prepare_command_mode_switch({"joint1/position", "joint2/effort"}, {}),
      false);
  ASSERT_EQ( // joint1 has two interfaces
      rm.prepare_command_mode_switch({"joint1/position", "joint1/acceleration"},
                                     {}),
      false);

  // switch controller mode as controller manager is doing - gpio itf 'vacuum'
  // will be ignored
  ASSERT_EQ(
  ...
  ASSERT_EQ(rm.prepare_command_mode_switch(
                {"joint1/velocity", "joint2/velocity"}, {}),
            true);
  ASSERT_EQ(rm.perform_command_mode_switch(
                {"joint1/velocity", "joint2/velocity"}, {}),
            true);
  ...
  // read() mirrors commands to states and calculate dynamics (both velocity
  // mode)
  rm.read(TIME, PERIOD);
  EXPECT_EQ(0.11, j1p_s.get_value());
...
TEST_F(TestGenericSystem,
       prepare_command_mode_switch_works_with_all_example_tags) {
  auto check_prepare_command_mode_switch =
      [&](const std::string &urdf,
          const std::string &urdf_head = ros2_control_test_assets::urdf_head) {
        TestableResourceManager rm(
            node_, urdf_head + urdf + ros2_control_test_assets::urdf_tail);
        ...
        return rm.prepare_command_mode_switch(start_interfaces,
                                              stop_interfaces);
      };

  ASSERT_TRUE(check_prepare_command_mode_switch(hardware_system_2dof_));
  ASSERT_TRUE(
      check_prepare_command_mode_switch(hardware_system_2dof_asymetric_));
  ASSERT_TRUE(check_prepare_command_mode_switch(
      hardware_system_2dof_standard_interfaces_));
  ASSERT_TRUE(check_prepare_command_mode_switch(
      hardware_system_2dof_with_other_interface_));
  ASSERT_TRUE(
      check_prepare_command_mode_switch(hardware_system_2dof_with_sensor_));
  ASSERT_TRUE(check_prepare_command_mode_switch(
      hardware_system_2dof_with_sensor_mock_command_));
  ASSERT_TRUE(check_prepare_command_mode_switch(
      hardware_system_2dof_with_sensor_mock_command_True_));
  ASSERT_TRUE(check_prepare_command_mode_switch(
      hardware_system_2dof_with_mimic_joint_,
      ros2_control_test_assets::urdf_head_mimic));
  ASSERT_TRUE(check_prepare_command_mode_switch(
      hardware_system_2dof_standard_interfaces_with_offset_));
  ASSERT_TRUE(check_prepare_command_mode_switch(
      hardware_system_2dof_standard_interfaces_with_custom_interface_for_offset_));
